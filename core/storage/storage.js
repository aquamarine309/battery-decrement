...
export const GameStorage = {
  ...
  get localStorageKey() {
    return DEV ? "batteryTestSave" : "batterySave";
  },

  backupDataKey(saveSlot, backupSlot) {
    return DEV ? `battery-backupTestSave-${saveSlot}-${backupSlot}` : `battery-backupSave-${saveSlot}-${backupSlot}`;
  },

  backupTimeKey(saveSlot) {
    return DEV ? `battery-backupTestTimes-${saveSlot}` : `battery-backupTimes-${saveSlot}`;
  },
  ...
  checkPlayerObject(save) {
    ...
    if (save.battery === undefined) return "Save does not have battery property";
    ...
    return `在player对象里发现了 ${formatInt(invalidProps.length)} 个NaN数据:
      ${invalidProps.join(", ")}`;
  },
...
(${this.exportDateString}).txt`, save);
  ...
  exportBackupsAsFile() {
    ...
    download(
      `Game Save Backups, Slot ${GameStorage.currentSlot + 1} #${player.options.exportedFileCount} \
